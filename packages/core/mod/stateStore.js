...
export const toStateStore = () => streamable => createStreamable(updates => using(scheduler => {
    ...
    const updatesSubscription = pipe(updates, zipWithLatestFrom(stream, (updateState, prev) => updateState(prev)), onNotify(dispatchTo(stream)), subscribe(scheduler), addDisposableOrTeardown(stream));
