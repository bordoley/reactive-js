...
const Observable_subscribeWithCapacityAndBackpressureStrategy: <T>(
  scheduler: SchedulerLike,
  capacity: number,
  backpressureStrategy: QueueableLike[typeof QueueableLike_backpressureStrategy],
) => Function1<ObservableLike<T>, DisposableLike> =
  (scheduler, capacity, backpressureStrategy) => observable =>
    pipe(
      Observer_create(scheduler, capacity, backpressureStrategy),
