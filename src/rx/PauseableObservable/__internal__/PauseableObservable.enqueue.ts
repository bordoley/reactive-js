...
import { Enqueue, PauseableObservableContainer } from "../../../rx.js";
...
const PauseableObservable_enqueue: Enqueue<PauseableObservableContainer>["enqueue"] =
  (<T>(queue: QueueableLike<T>) =>
    pipe(
      Observer_createEnqueueObserver,
      partial(queue),
      PauseableObservable_lift,
    )) as Enqueue<PauseableObservableContainer>["enqueue"];
