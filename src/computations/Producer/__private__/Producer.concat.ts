...
import * as DelegatingConsumer from "../../../utils/__internal__/DelegatingConsumer.js";
...
const Producer_concat: Producer.Signature["concat"] = /*@__PURE__*/ (<T>() => {
  ...
  function onConcatConsumerComplete(this: ConcatConsumerCtx) {
    ...
    if (next < producers[Array_length]) {
      ...
      producers[next][ProducerLike_consume](concatConsumer);
    } else {
  ...
  const createConcatConsumer = (ctx: ConcatConsumerCtx) => {
    ...
    return pipe(
      DelegatingConsumer.createNotifyOnlyNonCompletingNonDisposing(delegate),
  ...
  const createConcatProducer = mixInstanceFactory(
    function ConcatProducer(
      ...
      this[ComputationLike_isSynchronous] =
        Computation.areAllSynchronous(producers);
    ...
    }),
    {
      ...
      [ProducerLike_consume](
        ...
        pipe(
          ...
          bindMethod(producers[0], ProducerLike_consume),
        );
      },
    },
  );
