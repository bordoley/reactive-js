...
>(
  ...
  ConcurrentComputationModule<TComputationType> &
      ...
      testAsync("combineLatest from two interspersing sources", async () => {
        ...
        await pipeAsync(
          m.combineLatest<number, number>(
            pipe(
              Computation.generate<TComputationType>(m)(
                incrementBy(2),
                returns(1),
                { delay: 2 },
              ),
    ...
    describe(
      ...
      test("with pure src and inner runnables with side-effects", () => {
        ...
        pipe(
          ...
          m.fromObservable<number>({scheduler:vts}),
          ...
          Observable.subscribe({scheduler:vts}),
        );
        ...
        const [ev1, ev2, ev3] = pipe(
          ...
          >(
            compose(
              ...
              m.fromObservable({scheduler:vts}),
            ),
          ),
        );
      ...
      test("when one source throws", () => {
        ...
        const subscription = pipe(
          m.merge(
            ...
            pipe(Computation.raise(Observable)({ delay: 5 }), m.fromObservable({scheduler:vts})),
          ),
          m.toObservable(),
          Observable.subscribe({scheduler:vts}),
        );
    ...
    describe(
      ...
      test("when latest throws", () => {
        ...
        const result = pipe(
          ...
          m.withLatestFrom(
            pipe(
              Computation.raise<Observable.Computation>(Observable)<number>({ raise: returns(error) }),
              m.fromObservable<number>({scheduler:vts}),
            ),
            returns(1),
          ),
