/// <reference types="./Streamable.createAnimationsEventHandler.d.ts" />
...
const createAnimationsEventHandlerStream = 
/*@__PURE__*/ (() => {
    return createInstanceFactory(mix(include(Stream_delegatingMixin(), Delegating_mixin()), function AnimationEventHandlerStream(instance, animations, creationOptions, scheduler, streamOptions) {
        const streamDelegate = Streamable_createEventHandler((type) => {
            const observables = pipe(animations, ReadonlyObjectMap_mapWithKey((factory, key) => pipe(Observable_animate(factory(type)), Observable_map(value => ({ type, value })), Observable_forEach(value => {
...
const Streamable_createAnimationsEventHandler = ((animations, createOptions) => ({
    [StreamableLike_stream]: (scheduler, options) => createAnimationsEventHandlerStream(animations, createOptions, scheduler, options),
}));
