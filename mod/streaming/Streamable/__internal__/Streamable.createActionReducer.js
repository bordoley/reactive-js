...
const Streamable_createActionReducer = (reducer, initialState, options) => Streamable_createLifted(obs => Observable_create(observer => {
    ...
    pipe(obs, Observable_scan(reducer, returns(acc)), Observable_mergeWith(pipe([acc], ReadonlyArray_toRunnable())), Observable_distinctUntilChanged(options), Observable_observeWith(observer));
}));
