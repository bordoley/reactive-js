...
const Flowable_toObservable = () => src => Observable_create(observer => {
    ...
    const op = compose(Observable_forEach(Dispatcher_dispatchTo(dispatcher)), Observable_ignoreElements(), Observable_startWith(returns(PauseableState_paused), returns(PauseableState_running)), Observable_onSubscribe(() => dispatcher));
