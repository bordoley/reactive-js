...
const Flowable_createLifted = (...ops) => {
    const op = composeUnsafe(Observable_scan(updateReducer, returns(PauseableState_paused)), Observable_mergeWith(pipe([PauseableState_paused], ReadonlyArray_toRunnable())), Observable_distinctUntilChanged(), ...ops);
