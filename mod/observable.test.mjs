...
const tests = describe("observable", test("await_", defer([0, 1, 2, 3, 4], fromArray(), await_(compose(fromValue(), endWith(1))), toRunnable(), last, expectEquals(0))), describe("buffer", test("with duration and maxBufferSize", defer(concat(pipe([1, 2, 3, 4], fromArray()), pipe([1, 2, 3], fromArray({ delay: 1 })), pipe(4, fromValue({ delay: 8 }))), buffer({ duration: 4, maxBufferSize: 3 }), toRunnable(), toArray(), expectArrayEquals([[1, 2, 3], [4, 1, 2], [3], [4]], arrayEquality()))), test("when duration observable throws", defer(defer([1, 2, 3, 4], fromArray(), buffer({ duration: _ => throws()(raise) }), toRunnable({
...
})), test("combineLatest", defer(generate(incrementBy(2), returns(1), { delay: 2 }), takeFirst({ count: 3 }), combineLatestWith(pipe(generate(incrementBy(2), returns(0), { delay: 3 }), takeFirst({ count: 2 }))), toRunnable(), toArray(), expectArrayEquals([
    [3, 2],
    [5, 2],
    [5, 4],
    [7, 4],
...
}), describe("switchAll", test("with empty source", defer(empty(), switchAll(), toRunnable(), toArray(), expectArrayEquals([]))), test("when source throw", defer(defer(raise, throws(), switchAll(), toRunnable(), toArray(), expectArrayEquals([])), expectToThrow))), test("switchMap", defer([1, 2, 3], fromArray({ delay: 1 }), switchMap(_ => pipe([1, 2, 3], fromArray())), toRunnable(), toArray(), expectArrayEquals([1, 2, 3, 1, 2, 3, 1, 2, 3]))), describe("takeLast", test("when pipeline throws", defer(defer(raise, throws(), takeLast(), toRunnable(), last), expectToThrow))), describe("throttle", test("first", defer(generate(increment, returns(-1), { delay: 1 }), takeFirst({ count: 100 }), throttle(50, { mode: 1 /* First */ }), toRunnable(), toArray(), expectArrayEquals([0, 49]))), test("last", defer(generate(increment, returns(-1), { delay: 1 }), takeFirst({ count: 200 }), throttle(50, { mode: 2 /* Last */ }), toRunnable(), toArray(), expectArrayEquals([49, 99, 149, 199]))), test("interval", defer(generate(increment, returns(-1), { delay: 1 }), takeFirst({ count: 200 }), throttle(75, { mode: 3 /* Interval */ }), toRunnable(), toArray(), expectArrayEquals([0, 74, 149, 199]))), test("when duration observable throws", defer(defer([1, 2, 3, 4, 5], fromArray({ delay: 1 }), throttle(_ => throws()(raise)), toRunnable(), last), expectToThrow))), describe("throwIfEmpty", test("when source is empty", defer(defer(empty(), throwIfEmpty(() => undefined), toRunnable(), last), expectToThrow)), test("when source is not empty", defer(1, returns, compute(), throwIfEmpty(() => undefined), toRunnable(), last, expectEquals(1)))), describe("timeout", test("throws when a timeout occurs", defer(defer(1, fromValue({ delay: 2 }), timeout(1), toArray()), expectToThrow)), test("when timeout is greater than observed time", defer(1, fromValue({ delay: 2 }), timeout(3), toRunnable(), last, expectEquals(1)))), describe("toPromise", testAsync("when observable completes without producing a value", async () => {
    ...
    [2, 0],
    [3, 1],
], arrayEquality()))), test("when latest produces no values", defer([0], fromArray({ delay: 1 }), withLatestFrom(empty(), sum), toRunnable(), toArray(), expectArrayEquals([]))), test("when latest throws", () => {
    const error = new Error();
    pipe(defer([0], fromArray({ delay: 1 }), withLatestFrom(throws()(returns(error)), sum), toRunnable(), toArray(), expectArrayEquals([])), expectToThrowError(error));
})), describe("zip", test("with non-delayed sources", defer(zip(pipe([1, 2], fromArray()), pipe([1, 2], fromArray(), map(increment)), generate(increment, returns(2))), toRunnable(), toArray(), expectArrayEquals([
    [1, 2, 3],
    [2, 3, 4],
], arrayEquality()))), test("with synchronous and non-synchronous sources", defer(zip(pipe([1, 2], fromArray({ delay: 1 })), pipe([2, 3], fromIterable()), pipe([3, 4, 5], fromArray({ delay: 1 }))), toRunnable(), toArray(), expectArrayEquals([
    [1, 2, 3],
    [2, 3, 4],
], arrayEquality()))), test("fast with slow", defer([1, 2, 3], fromArray({ delay: 1 }), zipWith(pipe([1, 2, 3], fromArray({ delay: 5 }))), toRunnable(), toArray(), expectArrayEquals([
    [1, 1],
    [2, 2],
    [3, 3],
], arrayEquality()))), test("when source throws", defer(defer(raise, throws(), zipWith(fromArray()([1, 2, 3])), map(([, b]) => b), toRunnable(), toArray()), expectToThrow))), test("zipLatestWith", defer([1, 2, 3, 4, 5, 6, 7, 8], fromArray({ delay: 1 }), zipLatestWith(pipe([1, 2, 3, 4], fromArray({ delay: 2 }))), map(([a, b]) => a + b), toRunnable(), toArray(), expectArrayEquals([2, 5, 8, 11]))), describe("zipWithLatestFrom", test("when source throws", defer(defer(throws()(raise), zipWithLatestFrom(fromValue()(1), (_, b) => b), toRunnable(), last), expectToThrow)), test("when other throws", defer(defer([1, 2, 3], fromArray({ delay: 1 }), zipWithLatestFrom(throws()(raise), (_, b) => b), toRunnable(), last), expectToThrow)), test("when other completes first", defer([1], fromArray({ delay: 1 }), zipWithLatestFrom(fromArray()([2]), (_, b) => b), toRunnable(), last, expectEquals(2)))), createMonadTests(Observable));
