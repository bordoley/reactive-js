/// <reference types="./DisposableContainer.test.d.ts" />

import { describe, expectArrayEquals, expectEquals, expectIsNone, expectToHaveBeenCalledTimes, mockFn, test, testModule, } from "../../__internal__/testing.js";
...
testModule("DisposableContainer", describe("onComplete", test("disposing the parent without error invokes the callback", () => {
    ...
    const disposable = pipe(Disposable.create(), DisposableContainer.onDisposed(teardown));
    disposable[DisposableLike_dispose]();
    ...
    const disposable = pipe(Disposable.create(), DisposableContainer.onDisposed(childTeardown));
