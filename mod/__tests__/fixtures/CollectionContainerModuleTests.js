/// <reference types="./CollectionContainerModuleTests.d.ts" />
...
import { arrayEquality, compose, none, pipe, pipeLazy, returns, } from "../../functions.js";
import { CollectionLike_count, } from "../../types.js";
import ContainerModuleTests from "./ContainerModuleTests.js";
const CollectionContainerModuleTests = (m, fromReadonlyArray) => [
    ContainerModuleTests(m, () => Disposable.disposed, fromReadonlyArray, () => compose(m.values(), Observable.toReadonlyArray())),
    describe("CollectionContainerModuleTests", describe("empty", test("returns an empty enumerator", pipeLazy(m.empty(), m.values(), Observable.toReadonlyArray(), expectArrayEquals([])))), describe("entries", test("enumerates all entries", pipeLazy(["b", "d"], fromReadonlyArray(), m.entries(), Observable.toReadonlyArray(), expectArrayEquals([
        [0, "b"],
        [1, "d"],
    ], arrayEquality())))), describe("forEach", test("iterate and imperatively sum the keys", () => {
        let result = "";
        pipe(["b", "d", "v"], fromReadonlyArray(), m.forEach(value => {
            result = result + value;
        }));
        pipe(result, expectEquals("bdv"));
    })), describe("forEachWithKey", test("iterate and imperatively sum the keys", () => {
        let result = 0;
        pipe(["b", "d", "v"], fromReadonlyArray(), m.forEachWithKey((_, key) => {
            result = result + key;
        }));
    ...
    }), test("keys", pipeLazy(["b", none, "v"], fromReadonlyArray(), m.toDictionary(), Dictionary.keys(), Observable.toReadonlyArray(), expectArrayEquals([0, 1, 2])))), describe("toReadonlyMap", test("from non-empty map", () => {
        const dict = pipe(["b", "d", "v"], fromReadonlyArray(), m.toReadonlyMap());
        pipe(dict, ReadonlyMap.keys(), Observable.toReadonlyArray(), expectArrayEquals([0, 1, 2]));
    }))),
];
