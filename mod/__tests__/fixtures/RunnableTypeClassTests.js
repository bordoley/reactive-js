...
import { alwaysFalse, alwaysTrue, arrayEquality, greaterThan, none, pipe, pipeLazy, returns, } from "../../functions.js";
...
const RunnableTypeClassTests = (m) => describe("RunnableTypeClass", ...ContainerTypeClassTests(m, () => Disposable.disposed, () => (arr) => m.fromReadonlyArray()(arr), () => (c) => m.toReadonlyArray()(c)).tests, describe("concat", test("concats the input containers in order", pipeLazy(m.concat(pipe([1, 2, 3], m.fromReadonlyArray()), pipe([4, 5, 6], m.fromReadonlyArray())), m.toReadonlyArray(), expectArrayEquals([1, 2, 3, 4, 5, 6])))), describe("concatAll", test("concats the input containers in order", pipeLazy([
...
})), describe("fromFactory", test("it produces the factory result", pipeLazy(() => 1, m.fromFactory(), m.first(), expectEquals(1)))), describe("fromValue", test("it produces the value", pipeLazy(none, m.fromValue(), m.toReadonlyArray(), expectArrayEquals([none])))), describe("m.fromReadonlyArray", test("negative count with start index", () => {
