...
import { DisposableLike_isDisposed, ObservableLike_isDeferred, PauseableLike_resume, SchedulerLike_schedule, VirtualTimeSchedulerLike_run, } from "../../types.js";
import ContainerModuleTests from "./ContainerModuleTests.js";
const RunnableContainerModuleTests = (m) => [
    ContainerModuleTests(m, () => Disposable.disposed, () => (arr) => m.fromReadonlyArray()(arr), () => (c) => m.toReadonlyArray()(c)),
    describe("RunnableContainerModule", describe("concat", test("concats the input containers in order", pipeLazy(m.concat(pipe([1, 2, 3], m.fromReadonlyArray()), pipe([4, 5, 6], m.fromReadonlyArray())), m.toReadonlyArray(), expectArrayEquals([1, 2, 3, 4, 5, 6])))), describe("concatAll", test("concats the input containers in order", pipeLazy([
