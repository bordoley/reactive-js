...
const DeferredTypeClassTests = (m, toReadonlyArray) => [
    ContainerTypeClassTests(m, () => Disposable.disposed, () => (arr) => m.fromReadonlyArray()(arr), () => (c) => toReadonlyArray()(c)),
    describe("DeferredTypeClass", describe("concat", test("concats the input containers in order", pipeLazy(m.concat(pipe([1, 2, 3], m.fromReadonlyArray()), pipe([4, 5, 6], m.fromReadonlyArray())), toReadonlyArray(), expectArrayEquals([1, 2, 3, 4, 5, 6])))), describe("concatAll", test("concats the input containers in order", pipeLazy([
