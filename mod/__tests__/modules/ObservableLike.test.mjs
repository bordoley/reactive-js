...
import { pipeLazy, pipe, incrementBy, returns, getOrDefault, arrayEquality, getOrRaise, raise } from '../../functions.mjs';
import { generateObservable, emptyObservable } from '../../rx.mjs';
import { combineLatest, takeFirst, toRunnableObservable, share, zip, map, forEach, subscribe, toPromise, zipLatest } from '../../rx/ObservableLike.mjs';
...
const ObservableLikeTests = createDescribe("ObservableLike", createDescribe("combineLatest", createTest("combineLatest", pipeLazy(combineLatest(pipe(generateObservable(incrementBy(2), returns(1), { delay: 2 }), takeFirst({ count: 3 })), pipe(generateObservable(incrementBy(2), returns(0), { delay: 3 }), takeFirst({ count: 2 }))), toRunnableObservable(), getOrDefault(emptyObservable({ delay: 0 })), toReadonlyArray(), expectArrayEquals([
    [3, 2],
    [5, 2],
    [5, 4],
    [7, 4],
], arrayEquality())))), createDescribe("share", createTest("shared observable zipped with itself", () => {
