...
import { describe, expectArrayEquals, expectEquals, expectToThrowAsync, testAsync, testModule, } from "../__internal__/testing.js";
...
testModule("AsyncIterable", describe("flow", testAsync("infinite immediately resolving iterable", Disposable.usingAsyncLazy(Scheduler.createHostScheduler)(async (scheduler) => {
...
}), expectToThrowAsync))), describe("toObservable", testAsync("infinite immediately resolving iterable", Disposable.usingAsyncLazy(Scheduler.createHostScheduler)(async (scheduler) => {
    ...
    const result = await pipe((async function* foo() {
        throw e;
    })(), AsyncIterable.toObservable(), Observable.lastAsync(scheduler, { capacity: 1 }));
