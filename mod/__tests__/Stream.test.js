...
import { QueueableLike_enqueue, SchedulerLike_schedule, StreamableLike_stream, VirtualTimeSchedulerLike_run, } from "../types.js";
testModule("Stream", test("syncState", () => {
    ...
    pipe(stream, Stream.syncState(state => pipe(Observable.range(state + 10), Observable.delay(10), Observable.map(x => (_) => x), Observable.takeFirst({ count: 2 })), (oldState, newState) => newState !== oldState ? Observable.empty() : Observable.empty()));
