...
})), describe("pipeLazyAsync", testAsync("with pipeline that returns a value", pipeLazyAsync(1, bindMethod(Promise, "resolve"), expectEquals(1)))), describe("pipeSome", test("with some", pipeLazy(pipeSome(1, identity), expectIsSome)), test("with none", pipeLazy(pipeSome(none, identity), expectIsNone))), describe("pipeSomeLazy", test("with some", pipeLazy(pipeSomeLazy(1, identity)(), expectIsSome)), test("with none", pipeLazy(pipeSomeLazy(none, identity)(), expectIsNone))), describe("raiseIf", test("with true", () => {
