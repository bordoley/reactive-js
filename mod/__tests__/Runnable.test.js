...
import { describe, expectArrayEquals, expectToHaveBeenCalledTimes, expectToThrowAsync, expectTrue, mockFn, test, testModule, } from "../__internal__/testing.js";
import { arrayEquality, increment, isSome, none, pipe, pipeLazy, returns, } from "../functions.js";
...
testModule("Runnable", ...RunnableContainerModuleTests(Runnable), describe("compute", test("batch mode", pipeLazy(Runnable.compute(() => {
...
], Observable.fromReadonlyArray(), Runnable.exhaust(), Runnable.toReadonlyArray(), expectArrayEquals([1, 2, 3])))), describe("exhaustMap", test("when the initial observable never disposes", pipeLazy([1, 2, 3], Observable.fromReadonlyArray(), Runnable.exhaustMap(_ => pipe([1, 2, 3], Observable.fromReadonlyArray({ delay: 1 }))), Runnable.toReadonlyArray(), expectArrayEquals([1, 2, 3])))), describe("flow", test("a source with delay", () => {
...
})), describe("mergeMap", test("without delay, merge all observables as they are produced", pipeLazy([1, 2, 3], Observable.fromReadonlyArray(), Runnable.mergeMap(x => pipe([x, x, x], Observable.fromReadonlyArray())), Runnable.toReadonlyArray(), expectArrayEquals([1, 1, 1, 2, 2, 2, 3, 3, 3])))), describe("switchAll", test("with empty source", pipeLazy(Observable.empty(), Runnable.switchAll(), Runnable.toReadonlyArray(), expectArrayEquals([]))), test("when source throw", pipeLazy(pipeLazy(Observable.throws(), Runnable.switchAll(), Observable.toReadonlyArrayAsync()), expectToThrowAsync))), describe("switchMap", test("only produce the last observable", pipeLazy([1, 2, 3], Observable.fromReadonlyArray(), Runnable.switchMap(x => pipe([x, x, x], Observable.fromReadonlyArray({
    delay: 1,
    delayStart: true,
}))), Runnable.toReadonlyArray(), expectArrayEquals([3, 3, 3])))), describe("switchMap", test("overlapping notification", pipeLazy([none, none, none], Observable.fromReadonlyArray({ delay: 4 }), Runnable.switchMap(_ => pipe([1, 2, 3], Observable.fromReadonlyArray({ delay: 2 }))), Runnable.toReadonlyArray(), expectArrayEquals([1, 2, 1, 2, 1, 2, 3]))), test("concating arrays", pipeLazy([1, 2, 3], Observable.fromReadonlyArray({ delay: 1 }), Runnable.switchMap(_ => pipe([1, 2, 3], Observable.fromReadonlyArray({ delay: 0 }))), Runnable.toReadonlyArray(), expectArrayEquals([1, 2, 3, 1, 2, 3, 1, 2, 3])))));
((_) => { })(Runnable);
