...
testModule("Observable", ...RunnableContainerModuleTests({
    ...
    const result = [];
    pipe([9, 9, 9, 9], Observable.fromIterable(), Observable.delay(2), Observable.withCurrentTime(t => t), Observable.forEach(x => {
        result.push(x);
    }), Observable.run());
