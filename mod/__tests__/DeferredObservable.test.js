...
import { alwaysTrue, identityLazy, increment, pipe, pipeLazy, returns, } from "../functions.js";
...
testModule("DeferredObservable", HigherOrderObservableModuleTests(DeferredObservable, identityLazy), describe("retry", test("retrys the container on an exception", pipeLazy(Observable.concat(pipe(Observable.generate(increment, returns(0)), Observable.takeFirst({ count: 3 })), Observable.throws()), DeferredObservable.retry(alwaysTrue), Observable.takeFirst({ count: 6 }), Runnable.toReadonlyArray(), expectArrayEquals([1, 2, 3, 1, 2, 3])))), describe("share", test("shared observable zipped with itself", () => {
