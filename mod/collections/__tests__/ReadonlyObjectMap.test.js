...
testModule("ReadonlyObjectMap", DictionaryCollectionModuleTests(ReadonlyObjectMap), describe("empty", test("returns an empty enumerator", pipeLazy(ReadonlyObjectMap.empty(), ReadonlyObjectMap.values(), Enumerable.toReadonlyArray(), expectArrayEquals([])))), describe("entries", test("enumerates all entries", pipeLazy([tuple("0", "b"), tuple("1", "d")], ReadonlyArray.values(), ReadonlyObjectMap.fromEntries(), ReadonlyObjectMap.entries(), Enumerable.toReadonlyArray(), expectArrayEquals([tuple("0", "b"), tuple("1", "d")], {
    ...
    pipe(dict[DictionaryLike_keys], Enumerable.toReadonlyArray(), x => x[Array_length], expectEquals(3));
}), test("get values", () => {
    const dict = pipe([tuple("0", "b"), tuple("1", none), tuple("2", "v")], ReadonlyArray.values(), ReadonlyObjectMap.fromEntries(), ReadonlyObjectMap.toDictionary());
