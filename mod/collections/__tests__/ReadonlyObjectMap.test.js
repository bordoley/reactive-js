...
testModule("ReadonlyObjectMap", DictionaryCollectionModuleTests(ReadonlyObjectMap), describe("empty", test("returns an empty enumerator", pipeLazy(ReadonlyObjectMap.empty(), ReadonlyObjectMap.values(), ReadonlyArray.fromIterable(), expectArrayEquals([])))), describe("entries", test("enumerates all entries", pipeLazy([tuple("0", "b"), tuple("1", "d")], ReadonlyArray.values(), ReadonlyObjectMap.fromEntries(), ReadonlyObjectMap.entries(), ReadonlyArray.fromIterable(), expectArrayEquals([tuple("0", "b"), tuple("1", "d")], {
    valuesEquality: arrayEquality(),
})))), describe("map", test("mapping every value to its key", pipeLazy([tuple("0", "b"), tuple("1", "d"), tuple("2", "f")], ReadonlyArray.values(), ReadonlyObjectMap.fromEntries(), ReadonlyObjectMap.map((_, key) => key), ReadonlyObjectMap.values(), ReadonlyArray.fromIterable(), expectArrayEquals(["0", "1", "2"])))), describe("reduce", test("summing the keys", pipeLazy([tuple("0", "a"), tuple("1", "B"), tuple("2", "c")], ReadonlyArray.values(), ReadonlyObjectMap.fromEntries(), ReadonlyObjectMap.reduce((acc, value, _) => acc + value, returns("")), expectEquals("aBc")))), describe("toDictionary", test("count", () => {
    ...
    pipe(dict[DictionaryLike_keys], ReadonlyArray.fromIterable(), x => x[Array_length], expectEquals(3));
}), test("get values", () => {
    const dict = pipe([tuple("0", "b"), tuple("1", none), tuple("2", "v")], ReadonlyArray.values(), ReadonlyObjectMap.fromEntries(), ReadonlyObjectMap.toDictionary());
    pipe(dict, Dictionary.values(), ReadonlyArray.fromIterable(), expectArrayEquals(["b", none, "v"]));
}), test("keys", pipeLazy([tuple("0", "b"), tuple("1", none), tuple("2", "v")], ReadonlyArray.values(), ReadonlyObjectMap.fromEntries(), ReadonlyObjectMap.toDictionary(), Dictionary.keys(), ReadonlyArray.fromIterable(), expectArrayEquals(["0", "1", "2"]))), test("get returns none if a key is missing", () => {
