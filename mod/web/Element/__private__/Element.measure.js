...
const Element_measure = /*@__PURE__*/ (() => {
    ...
    return returns((element) => measureStoreCache.get(element) ??
        (() => {
            ...
            pipe(Broadcaster.merge(windowResizeEventSource, windowScrollEventSource, ...scrollContainerEventSources), Broadcaster.map(pipeLazy(element, measureElement)), invoke(EventSourceLike_subscribe, store));
            return store;
        })());
})();
