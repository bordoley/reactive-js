...
const PauseableObservable_create = /*@__PURE__*/ (() => {
    return createInstanceFactory(mix(include(Disposable_delegatingMixin, Delegating_mixin(), EventPublisher_lazyInitMixin()), function PauseableObservable(instance, op, scheduler, multicastOptions) {
        ...
        pipe(true, Optional_toObservable())), Observable_distinctUntilChanged(), Observable_forEach(isPaused => notifyPauseState(isPaused)), op);
        ...
        const notifyPauseState = (isPause) => {
            instance[PauseableLike_isPaused] = isPause;
            instance[EventListenerLike_notify](isPause ? { type: "paused" } : { type: "resumed" });
        };
        return instance;
    }, props({
