...
const HigherOrderObservableLike__scanAsync = (createObservable) => {
    return (scanner, initialValue) => observable => {
        const onSink = (observer) => {
            ...
            pipe(observable, ObservableLike__zipWithLatestFrom(accFeedbackStream, (next, acc) => pipe(scanner(acc, next), ObservableLike__takeFirst())), ObservableLike__switchAll(), ObservableLike__forEach(publishTo(accFeedbackStream)), ObservableLike__onSubscribe(() => pipe(accFeedbackStream, publish(initialValue()))), ReactiveContainerLike__sinkInto(observer));
        };
