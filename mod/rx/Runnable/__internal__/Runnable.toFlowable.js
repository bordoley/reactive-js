...
const Runnable_toFlowable = () => observable => Flowable_createLifted((modeObs) => Observable_create(observer => {
    ...
    }), Observable_subscribeWithCapacityAndBackpressureStrategy(observer[DispatcherLike_scheduler], observer[QueueableLike_capacity], observer[QueueableLike_backpressureStrategy]), Disposable_bindTo(pauseableScheduler))), Disposable_add(pauseableScheduler));
}), true);
