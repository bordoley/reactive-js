/// <reference types="./Observable.scanWithSpring.d.ts" />
...
const Observable_scanWithSpring = (initialValue, options) => {
    ...
    return compose(Observable_withCurrentTime((time, next) => [time, next]), Observable_scanMany((prev, [start, next]) => pipe(Observable_currentTime(), Observable_scan(([lastTime, last, value], now) => {
