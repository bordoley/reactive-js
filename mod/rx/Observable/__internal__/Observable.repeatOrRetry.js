    ...
    const createRepeatObserver = (delegate, observable, shouldRepeat) => {
        ...
        const doOnDispose = (err) => {
            ...
            if (shouldComplete) {
                ...
                count++;
                pipe(observable, Observable_forEach(Observer_notifySink(delegate)), Observable_subscribe(Observer_getScheduler(delegate)), Disposable_addToIgnoringChildErrors(delegate), Disposable_onDisposed(doOnDispose));
            }
        };
