...
const HigherOrderObservable_scanLast = (createObservable) => (scanner, initialValue) => observable => createObservable((observer) => {
    ...
    pipe(observable, Observable_zipWithLatestFrom(accFeedbackStream, (next, acc) => pipe(scanner(acc, next), Observable_takeLast())), Observable_concatAll(), Observable_forEach(bindMethod(accFeedbackStream, PublisherLike_publish)), Observable_observeWith(observer));
