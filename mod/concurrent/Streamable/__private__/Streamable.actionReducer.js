...
const Observable_actionReducer = (reducer, initialState, options) => (obs) => Observable.create((observer) => {
    ...
    return pipe(obs, Observable.scan(reducer, returns(acc)), Observable.mergeWith(pipe(acc, Observable.fromValue())), Observable.distinctUntilChanged(options), invoke(ObservableLike_observe, observer));
});
