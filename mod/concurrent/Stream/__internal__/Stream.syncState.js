...
const Stream_syncState = (onInit, onChange, options) => {
    ...
    return (stateStore) => {
        ...
        return pipe(stateStore, Observable.forkMerge(compose(Observable.takeFirst(), Observable.concatMap(onInit)), compose(throttleDuration > 0
            ? Observable.throttle(throttleDuration)
            : identity, Observable.pairwise(), Observable.concatMap(([oldValue, newValue]) => onChange(oldValue, newValue)))), Observable.dispatchTo(stateStore), Observable.subscribe(scheduler, {
