...
const PureObservableOperatorWithDeferredObservableBaseTests = (op) => describe("PureObservableOperatorWithDeferredObservableBaseTests", test("with PureRunnableLike", pipeLazy(Observable.empty(), op, expectIsPureRunnable)), test("with RunnableWithSideEffectsLike", pipeLazy(Observable.empty(), Observable.forEach(ignore), op, expectIsRunnableWithSideEffects)), test("with PureDeferredObservableLike", Disposable.usingLazy(VirtualTimeScheduler.create)(vts => pipe(Observable.empty(), Observable.subscribeOn(vts), op, expectIsPureDeferredObservable))), test("with DeferredObservableWithSideEffectsLike", pipeLazy(async () => {
    throw new Error();
}, Observable.fromAsyncFactory(), op, expectIsDeferredObservableWithSideEffects)));
const PureDeferredObservableOperatorTests = (op) => describe("PureObservableOperator", test("with PureRunnableLike", pipeLazy(Observable.empty(), op, expectIsPureDeferredObservable)), test("with RunnableWithSideEffectsLike", pipeLazy(Observable.empty(), Observable.forEach(ignore), op, expectIsDeferredObservableWithSideEffects)), test("with PureDeferredObservableLike", Disposable.usingLazy(VirtualTimeScheduler.create)(vts => pipe(Observable.empty(), Observable.subscribeOn(vts), op, expectIsPureDeferredObservable))), test("with DeferredObservableWithSideEffectsLike", pipeLazy(async () => {
    throw new Error();
}, Observable.fromAsyncFactory(), op, expectIsDeferredObservableWithSideEffects)), test("with MulticastObservableLike", pipeLazy(new Promise(ignore), Observable.fromPromise(), op, expectIsMulticastObservable)));
...
const MulticastObservableOperatorTests = (op) => describe("MulticastObservableOperatorTests", test("with PureRunnableLike", pipeLazy(Observable.empty(), op, expectIsMulticastObservable)), test("with RunnableWithSideEffectsLike", pipeLazy(Observable.empty(), Observable.forEach(ignore), op, expectIsDeferredObservableWithSideEffects)), test("with PureDeferredObservableLike", Disposable.usingLazy(VirtualTimeScheduler.create)(vts => pipe(Observable.empty(), Observable.subscribeOn(vts), op, expectIsMulticastObservable))), test("with DeferredObservableWithSideEffectsLike", pipeLazy(async () => {
    throw new Error();
}, Observable.fromAsyncFactory(), op, expectIsDeferredObservableWithSideEffects)), test("with MulticastObservableLike", pipeLazy(new Promise(ignore), Observable.fromPromise(), op, expectIsMulticastObservable)));
...
testModule("Observable", PureComputationModuleTests(Observable, Observable.toReadonlyArray), describe("animate", testIsPureRunnable(Observable.animate([
    ...
    pipe(result, ReadonlyArray.map(x => x.message), expectArrayEquals(["e2", "e1"]));
}), PureObservableOperatorTests(Observable.catchError(ignore))), describe("combineLatest", test("combineLatest", pipeLazy(Observable.combineLatest(pipe(Enumerable.generate(incrementBy(2), returns(1)), Observable.fromEnumerable({ delay: 2 }), Observable.takeFirst({ count: 3 })), pipe(Enumerable.generate(incrementBy(2), returns(0)), Observable.fromEnumerable({ delay: 3 }), Observable.takeFirst({ count: 2 }))), Observable.toReadonlyArray(), expectArrayEquals([tuple(3, 2), tuple(5, 2), tuple(5, 4), tuple(7, 4)], { valuesEquality: arrayEquality() }))), testIsPureRunnable(Observable.combineLatest(Observable.empty(), Observable.empty())), testIsPureDeferredObservable(Disposable.using(VirtualTimeScheduler.create)(vts => Observable.combineLatest(pipe(Observable.empty(), Observable.subscribeOn(vts)), Observable.empty()))), testIsRunnableWithSideEffects(Observable.combineLatest(Observable.empty(), pipe(Observable.empty(), Observable.forEach(ignore)))), testIsMulticastObservable(Observable.combineLatest(Observable.empty(), Subject.create())), testIsDeferredObservableWithSideEffects(Observable.combineLatest(pipe(async () => {
    throw new Error();
}, Observable.fromAsyncFactory()), Observable.empty(), pipe(Observable.empty(), Observable.forEach(ignore))))), describe("computeDeferred", testAsync("__stream", Disposable.usingAsyncLazy(HostScheduler.create)(async (scheduler) => pipeAsync(Observable.computeDeferred(() => {
...
}))), describe("concatMany", test("concating an empty array returns the empty observable", pipeLazy(Observable.concatMany([]), expectEquals(Observable.empty()))), testIsPureRunnable(Observable.concatMany([Observable.empty(), Observable.empty()])), testIsPureDeferredObservable(Disposable.using(VirtualTimeScheduler.create)(vts => Observable.concatMany([
    ...
    Observable.empty(),
])), testIsDeferredObservableWithSideEffects(Observable.concatMany([Observable.create(ignore), Observable.empty()])), testIsMulticastObservable(Observable.concatMany([Subject.create(), Observable.empty()])), testIsDeferredObservableWithSideEffects(Observable.concatMany([
    Subject.create(),
    pipe(Observable.empty(), Observable.forEach(ignore)),
...
}), Observable.toReadonlyArray(), expectArrayEquals([1, 2, 3, 1, 2, 3]))), PureObservableOperatorTests(Observable.concatMap(_ => Observable.empty())), PureDeferredObservableOperatorTests(Observable.concatMap(_ => Observable.empty(), {
    innerType: Observable.PureDeferredObservableType,
})), ObservableOperatorWithSideEffectsTests(Observable.concatMap(_ => pipe(Observable.empty(), Observable.forEach(ignore)), {
    innerType: Observable.RunnableWithSideEffectsType,
})), AlwaysReturnsDeferredObservableWithSideEffectsOperatorTests(Observable.concatMap(_ => pipe(Observable.empty(), Observable.forEach(ignore)), {
    innerType: Observable.DeferredObservableWithSideEffectsType,
}))), describe("concatWith", test("concats two containers together", pipeLazy([0, 1], Observable.fromReadonlyArray(), Observable.concatWith(pipe([2, 3, 4], Observable.fromReadonlyArray())), Observable.toReadonlyArray(), expectArrayEquals([0, 1, 2, 3, 4]))), PureObservableOperatorTests(Observable.concatWith(Observable.empty(), Observable.empty())), ObservableOperatorWithSideEffectsTests(Observable.concatWith(pipe(Observable.empty(), Observable.forEach(ignore)))), PureDeferredObservableOperatorTests(Disposable.using(VirtualTimeScheduler.create)(vts => Observable.concatWith(pipe(Observable.empty(), Observable.subscribeOn(vts))))), describe("concat with DeferredObservableWithSideEffectsLikes", test("with PureRunnableLike", pipeLazy(Observable.empty(), Observable.concatWith(pipe(async () => {
    throw new Error();
...
expectArrayEquals([0, 0, 0, 0, 0]))), testIsPureRunnable(Observable.currentTime)), describe("debug", ObservableOperatorWithSideEffectsTests(Observable.debug())), describe("decodeWithCharset", PureObservableOperatorTests(Observable.decodeWithCharset())), describe("defer", testAsync("defering a promise converted to an Observable", Disposable.usingAsyncLazy(HostScheduler.create)(scheduler => pipeAsync(Observable.defer(() => pipe(Promise.resolve(1), Observable.fromPromise())), Observable.toReadonlyArrayAsync(scheduler), expectArrayEquals([1])))), testIsDeferredObservableWithSideEffects(Observable.defer(Subject.create))), describe("dispatchTo", test("when backpressure exception is thrown", Disposable.usingLazy(VirtualTimeScheduler.create)(vts => {
    ...
    innerType: Observable.PureRunnableType,
}), Observable.toReadonlyArray(), expectArrayEquals([1, 2, 3]))), PureObservableOperatorTests(Observable.exhaust()), PureDeferredObservableOperatorTests(Observable.exhaust({
    innerType: Observable.PureDeferredObservableType,
    ...
    innerType: Observable.PureRunnableType,
}), Observable.toReadonlyArray(), expectArrayEquals([1, 2, 3]))), PureObservableOperatorTests(Observable.exhaustMap(_ => Observable.empty())), PureDeferredObservableOperatorTests(Observable.exhaustMap(_ => Observable.empty(), {
    innerType: Observable.PureDeferredObservableType,
})), ObservableOperatorWithSideEffectsTests(Observable.exhaustMap(_ => pipe(Observable.empty(), Observable.forEach(ignore)), {
    innerType: Observable.RunnableWithSideEffectsType,
})), AlwaysReturnsDeferredObservableWithSideEffectsOperatorTests(Observable.exhaustMap(_ => pipe(Observable.empty(), Observable.forEach(ignore)), {
    innerType: Observable.DeferredObservableWithSideEffectsType,
...
})), describe("type tests", describe("with PureRunnable inner functions", test("with PureRunnable source", pipeLazy(Observable.empty(), Observable.forkMerge(returns(Observable.empty()), returns(Observable.empty())), expectIsPureRunnable)), test("with RunnableWithSideEffects source", pipeLazy(Observable.empty(), Observable.forEach(ignore), Observable.forkMerge(returns(Observable.empty()), returns(Observable.empty())), expectIsPureRunnable)), test("with DeferredObservableWithSideEffects source", pipeLazy(async () => {
    throw new Error();
}, Observable.fromAsyncFactory(), Observable.forkMerge(returns(Observable.empty()), returns(Observable.empty())), expectIsPureRunnable)), test("with MulticastObservable source", pipeLazy(Subject.create(), Observable.forkMerge(returns(Observable.empty()), returns(Observable.empty())), expectIsPureRunnable))), describe("with PureDeferredObservable inner functions", test("with PureRunnable source", Disposable.usingLazy(VirtualTimeScheduler.create)(vts => pipe(Observable.empty(), Observable.forkMerge(pipe(Observable.empty(), Observable.subscribeOn(vts), returns), returns(Observable.empty())), expectIsPureDeferredObservable))), test("with RunnableWithSideEffects source", Disposable.usingLazy(VirtualTimeScheduler.create)(vts => pipe(Observable.empty(), Observable.forEach(ignore), Observable.forkMerge(pipe(Observable.empty(), Observable.subscribeOn(vts), returns), returns(Observable.empty())), expectIsPureDeferredObservable))), test("with DeferredObservableWithSideEffects source", Disposable.usingLazy(VirtualTimeScheduler.create)(vts => pipe(async () => {
    throw new Error();
}, Observable.fromAsyncFactory(), Observable.forkMerge(pipe(Observable.empty(), Observable.subscribeOn(vts), returns), returns(Observable.empty())), expectIsPureDeferredObservable))), test("with MulticastObservable source", Disposable.usingLazy(VirtualTimeScheduler.create)(vts => pipe(Subject.create(), Observable.forkMerge(pipe(Observable.empty(), Observable.subscribeOn(vts), returns), returns(Observable.empty())), expectIsPureDeferredObservable)))), describe("with RunnableWithSideEffects inner functions", test("with PureRunnable source", pipeLazy(Observable.empty(), Observable.forkMerge(pipe(Observable.empty(), Observable.forEach(ignore), returns), returns(Observable.empty())), expectIsRunnableWithSideEffects)), test("with RunnableWithSideEffects source", pipeLazy(Observable.empty(), Observable.forEach(ignore), Observable.forkMerge(pipe(Observable.empty(), Observable.forEach(ignore), returns), returns(Observable.empty())), expectIsRunnableWithSideEffects)), test("with DeferredObservableWithSideEffects source", pipeLazy(async () => {
    throw new Error();
}, Observable.fromAsyncFactory(), Observable.forkMerge(pipe(Observable.empty(), Observable.forEach(ignore), returns), returns(Observable.empty())), expectIsRunnableWithSideEffects)), test("with MulticastObservable source", pipeLazy(Subject.create(), Observable.forkMerge(pipe(Observable.empty(), Observable.forEach(ignore), returns), returns(Observable.empty())), expectIsRunnableWithSideEffects))), describe("with PureObservable inner functions", test("with PureRunnable source", pipeLazy(Observable.empty(), Observable.forkMerge(() => Subject.create(), returns(Observable.empty())), expectIsMulticastObservable)), test("with RunnableWithSideEffects source", pipeLazy(Observable.empty(), Observable.forEach(ignore), Observable.forkMerge(() => Subject.create(), returns(Observable.empty())), expectIsMulticastObservable)), test("with DeferredObservableWithSideEffects source", pipeLazy(async () => {
    throw new Error();
}, Observable.fromAsyncFactory(), Observable.forkMerge(() => Subject.create(), returns(Observable.empty())), expectIsMulticastObservable)), test("with MulticastObservable source", pipeLazy(Subject.create(), Observable.forkMerge(() => Subject.create(), returns(Observable.empty())), expectIsMulticastObservable))), describe("with mixed inner functions", test("with PureRunnable source", pipeLazy(Observable.empty(), Observable.forkMerge(pipe(() => Promise.resolve(1), Observable.fromAsyncFactory(), returns), () => Subject.create(), returns(Observable.empty()), pipe(Observable.empty(), Observable.forEach(ignore), returns)), expectIsDeferredObservableWithSideEffects)), test("with RunnableWithSideEffects source", pipeLazy(Observable.empty(), Observable.forEach(ignore), Observable.forkMerge(pipe(() => Promise.resolve(1), Observable.fromAsyncFactory(), returns), () => Subject.create(), returns(Observable.empty()), pipe(Observable.empty(), Observable.forEach(ignore), returns)), expectIsDeferredObservableWithSideEffects)), test("with DeferredObservableWithSideEffects source", pipeLazy(async () => {
    throw new Error();
}, Observable.fromAsyncFactory(), Observable.forkMerge(pipe(() => Promise.resolve(1), Observable.fromAsyncFactory(), returns), () => Subject.create(), returns(Observable.empty()), pipe(Observable.empty(), Observable.forEach(ignore), returns)), expectIsDeferredObservableWithSideEffects)), test("with MulticastObservable source", pipeLazy(Subject.create(), Observable.forkMerge(pipe(() => Promise.resolve(1), Observable.fromAsyncFactory(), returns), () => Subject.create(), returns(Observable.empty()), pipe(Observable.empty(), Observable.forEach(ignore), returns)), expectIsDeferredObservableWithSideEffects))))), describe("fromAsyncFactory", testAsync("when promise resolves", Disposable.usingAsyncLazy(HostScheduler.create)(scheduler => pipeAsync(async () => {
    ...
    concurrency: 2,
    innerType: Observable.PureRunnableType,
}), Observable.toReadonlyArray(), expectArrayEquals([1, 2, 3, 4, 5, 6, 9, 10]))), PureObservableOperatorTests(Observable.mergeAll()), PureDeferredObservableOperatorTests(Observable.mergeAll({
    innerType: Observable.PureDeferredObservableType,
    ...
    innerType: Observable.DeferredObservableWithSideEffectsType,
}))), describe("mergeMany", testIsPureRunnable(Observable.mergeMany([Observable.empty(), Observable.empty()])), testIsPureDeferredObservable(Disposable.using(VirtualTimeScheduler.create)(vts => Observable.mergeMany([
    pipe(Observable.empty(), Observable.subscribeOn(vts)),
    Observable.empty(),
    ...
    innerType: Observable.PureRunnableType,
}), Observable.toReadonlyArray(), expectArrayEquals([1, 1, 1, 2, 2, 2, 3, 3, 3]))), PureObservableOperatorTests(Observable.mergeMap(_ => Observable.empty())), PureDeferredObservableOperatorTests(Observable.mergeMap(_ => Observable.empty(), {
    innerType: Observable.PureDeferredObservableType,
})), ObservableOperatorWithSideEffectsTests(Observable.mergeMap(_ => pipe(Observable.empty(), Observable.forEach(ignore)), {
    innerType: Observable.RunnableWithSideEffectsType,
    ...
    innerType: Observable.PureRunnableType,
}), Observable.toReadonlyArray(), expectArrayEquals([]))), PureObservableOperatorTests(Observable.switchAll()), PureDeferredObservableOperatorTests(Observable.switchAll({
    innerType: Observable.PureDeferredObservableType,
...
}), PureObservableOperatorTests(Observable.withLatestFrom(Observable.empty(), returns)), ObservableOperatorWithSideEffectsTests(Observable.withLatestFrom(pipe(Observable.empty(), Observable.forEach(ignore)), returns)), AlwaysReturnsDeferredObservableWithSideEffectsOperatorTests(Observable.withLatestFrom(pipe(() => Promise.resolve(1), Observable.fromAsyncFactory(), Observable.forEach(ignore)), returns)), MulticastObservableOperatorTests(Observable.withLatestFrom(Subject.create(), returns))), describe("zipLatest", test("zip two delayed observable", pipeLazy(Observable.zipLatest(pipe([1, 2, 3, 4, 5, 6, 7, 8], Observable.fromReadonlyArray({ delay: 1, delayStart: true })), pipe([1, 2, 3, 4], Observable.fromReadonlyArray({ delay: 2, delayStart: true }))), Observable.map(([a, b]) => a + b), Observable.toReadonlyArray(), expectArrayEquals([2, 5, 8, 11]))), testIsPureRunnable(Observable.zipLatest(Observable.empty(), Observable.empty())), testIsPureDeferredObservable(Disposable.using(VirtualTimeScheduler.create)(vts => Observable.zipLatest(pipe(Observable.empty(), Observable.subscribeOn(vts)), Observable.empty()))), testIsRunnableWithSideEffects(Observable.zipLatest(Observable.empty(), pipe(Observable.empty(), Observable.forEach(ignore)))), testIsMulticastObservable(Observable.zipLatest(Observable.empty(), Subject.create())), testIsDeferredObservableWithSideEffects(Observable.zipLatest(pipe(async () => {
    throw new Error();
}, Observable.fromAsyncFactory()), Observable.empty(), pipe(Observable.empty(), Observable.forEach(ignore))))));
((_) => { })(Observable);
