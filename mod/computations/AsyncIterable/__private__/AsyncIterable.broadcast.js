/// <reference types="./AsyncIterable.broadcast.d.ts" />

import { Iterator_done, Iterator_next, Iterator_value, MAX_SAFE_INTEGER, } from "../../../__internal__/constants.js";
import { error, invoke, none, pipe, pipeSome } from "../../../functions.js";
import * as Disposable from "../../../utils/Disposable.js";
import { DisposableLike_dispose, DisposableLike_isDisposed, EventListenerLike_notify, SchedulerLike_maxYieldInterval, SchedulerLike_now, SchedulerLike_schedule, } from "../../../utils.js";
...
const AsyncIterable_broadcast = (options) => (iterable) => Broadcaster_createPauseable((modeObs) => pipe(Broadcaster_create((listener) => {
    ...
    const continuation = async () => {
        ...
        pipeSome(!isPaused ? scheduler : none, invoke(SchedulerLike_schedule, continuation), Disposable.addTo(listener));
    };
    pipe(modeObs, Broadcaster_addEventHandler((mode) => {
        const wasPaused = isPaused;
        isPaused = mode;
        if (!isPaused && wasPaused) {
            pipeSome(scheduler, invoke(SchedulerLike_schedule, continuation), Disposable.addTo(listener)) ?? continuation();
        }
    }), Disposable.bindTo(listener));
}, options), Disposable.addTo(modeObs)));
