...
import { Computation_deferredWithSideEffectsOfT, Computation_pureDeferredOfT, Computation_pureSynchronousOfT, Computation_synchronousWithSideEffectsOfT, DeferredComputationWithSideEffects, ProducerLike_consume, PureDeferredComputation, PureSynchronousDeferredComputation, StoreLike_value, StreamableLike_stream, SynchronousDeferredComputationWithSideEffects, } from "../../computations.js";
import { bindMethod, error, ignore, invoke, isSome, lessThan, newInstance, none, pipe, pipeAsync, pipeLazy, pipeLazyAsync, raise, returns, tuple, } from "../../functions.js";
...
testModule("Observable", describe("effects", test("calling an effect from outside a computation expression throws", () => {
    ...
    capacity: 10,
    backpressureStrategy: DropLatestBackpressureStrategy,
}))), describe("broadcast", test("a source with delay", () => {
    ...
    try {
        ...
        const f = mockFn();
        const subscription = pipe(generateObservable, Broadcaster.toObservable(), Observable.forEach((x) => {
            f(x);
    ...
    try {
        ...
        const flowed = pipe([0, 1, 2], Observable.fromReadonlyArray(), Observable.broadcast(vts), Disposable.addTo(vts));
        vts[SchedulerLike_schedule](() => flowed[PauseableLike_resume](), {
            delay: 2,
        });
        const f = mockFn();
        const subscription = pipe(flowed, Broadcaster.toObservable(), Observable.withCurrentTime(tuple), Observable.forEach(([_, v]) => {
            f(v);
    ...
    try {
        ...
        const flowed = pipe(Observable.raise({ raise: () => error }), Observable.broadcast(vts), Disposable.addTo(vts));
...
expectArrayEquals([0, 0, 0, 0, 0]))), ComputationTest.isPureSynchronous(Observable.currentTime)), describe("defer", testAsync("defering a promise converted to an Observable", async () => {
    ...
    try {
        ...
        pipe(Computation.generate(Observable)(increment, returns(-1), { delay: 1, delayStart: true }), Observable.takeFirst({ count: 5 }), Observable.toEventSource(vts), EventSource.toProducer(), invoke(ProducerLike_consume, dest));
        const result = [];
