...
const SequentialReactiveComputationModuleTests = (m) => describe("SequentialReactiveComputationModule", describe("buffer", testAsync("with multiple sub buffers", pipeLazyAsync([1, 2, 3, 4, 5, 6, 7, 8, 9], Computation.fromReadonlyArray(m)(), m.buffer({ count: 3 }), Computation.toReadonlyArrayAsync(m)(), expectArrayEquals([
    [1, 2, 3],
    [4, 5, 6],
    [7, 8, 9],
], { valuesEquality: arrayEquality() }))), testAsync("last buffer is short", pipeLazyAsync([1, 2, 3, 4, 5, 6, 7, 8], Computation.fromReadonlyArray(m)(), m.buffer({ count: 3 }), Computation.toReadonlyArrayAsync(m)(), expectArrayEquals([
    [1, 2, 3],
    [4, 5, 6],
    [7, 8],
], { valuesEquality: arrayEquality() }))), testAsync("buffers all values when no count is provided", pipeLazyAsync([1, 2, 3, 4, 5, 6, 7, 8], Computation.fromReadonlyArray(m)(), m.buffer(), Computation.toReadonlyArrayAsync(m)(), expectArrayEquals([[1, 2, 3, 4, 5, 6, 7, 8]], {
    valuesEquality: arrayEquality(),
})))), describe("decodeWithCharset", testAsync("decoding ascii", async () => {
    const str = "abcdefghijklmnsopqrstuvwxyz";
    await pipeAsync([str], Computation.fromReadonlyArray(m)(), m.encodeUtf8(), m.decodeWithCharset(), Computation.toReadonlyArrayAsync(m)(), invoke("join"), expectEquals(str));
