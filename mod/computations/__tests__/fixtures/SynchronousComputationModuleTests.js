...
}), test("when error handler returns a computation", pipeLazy([1, 2, 3], m.fromReadonlyArray(), Computation.concatWith(m)(m.raise()), m.catchError(pipeLazy([4, 5, 6], m.fromReadonlyArray())), m.toReadonlyArray(), expectArrayEquals([1, 2, 3, 4, 5, 6])))), describe("concatAll", test("concating inner sources", pipeLazy([
