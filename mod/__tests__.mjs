...
import { identity as identity$1, __stream, createActionReducer, stream, empty as empty$1, createLiftedStreamable, sourceFrom, flow, flowToObservable } from './streamable.mjs';
...
const tests$1 = describe("streamable", test("__stream", () => {
    ...
    let result = 0;
    const dest = pipe(createLiftedStreamable(scan$2((acc, _) => acc + 1, returns(0)), onNotify(v => {
        result = v;
    }), mapTo(mapT, none), startWith({ ...concatT, ...fromArrayT$2 }, none)), stream(scheduler), sourceFrom(src));
