...
import { fromValue, empty, endWith, concatMap, mapTo, startWith, ignoreElements, contains, compute, noneSatisfy, zipWith, throws, concatWith } from './container.mjs';
...
import { onNotify, subscribe, toRunnable, fromArrayT, concat as concat$2, fromArray as fromArray$3, buffer, catchError, generate as generate$3, takeFirst as takeFirst$2, combineLatestT, createObservable, Subject, publishTo, getObserverCount, exhaustT, mapT as mapT$1, fromPromise, toPromise, concatAllT, fromIteratorT, merge, mergeT, mergeAllT, never, observable, __memo, __observe, takeLast as takeLast$2, onSubscribe, retry, scanAsync as scanAsync$1, share, zip as zip$1, map as map$3, switchAll, switchAllT, throttle, throwIfEmpty, timeout, withLatestFrom, fromIterable as fromIterable$2, zipT, zipLatestT, zipWithLatestFrom, keepT as keepT$3, scan as scan$4, concatT as concatT$1, decodeWithCharset, reduce, deferT } from './observable.mjs';
...
import { genMap, encodeUtf8 } from './liftableContainer.mjs';
import { fromArrayT as fromArrayT$4, mapT, concatT, concatAllT as concatAllT$1, keepT as keepT$2, distinctUntilChangedT, generateT, repeatT, scanT, skipFirstT, takeFirstT, takeLastT, takeWhileT, toRunnableT, zipT as zipT$1 } from './runnableObservable.mjs';
import { TContainerOf, fromArray as fromArray$4, concat as concat$3, concatAll as concatAll$2, distinctUntilChanged as distinctUntilChanged$2, generate as generate$4, keep as keep$1, map as map$4, repeat as repeat$2, scan as scan$3, skipFirst as skipFirst$2, takeFirst as takeFirst$3, takeLast as takeLast$3, takeWhile as takeWhile$3, toRunnable as toRunnable$3, fromArrayT as fromArrayT$5, zipT as zipT$2 } from './sequence.mjs';
...
const tests$3 = describe("observable", describe("buffer", test("with duration and maxBufferSize", pipeLazy(concat$2(pipe([1, 2, 3, 4], fromArray$3()), pipe([1, 2, 3], fromArray$3({ delay: 1 })), pipe(4, fromValue(fromArrayT$4, { delay: 8 }))), buffer({ duration: 4, maxBufferSize: 3 }), toRunnable(), toArray(), expectArrayEquals([[1, 2, 3], [4, 1, 2], [3], [4]], arrayEquality()))), test("when duration observable throws", pipeLazy(pipeLazy([1, 2, 3, 4], fromArray$3(), buffer({ duration: _ => throws({ ...fromArrayT$4, ...mapT })(raise) }), toRunnable({
    schedulerFactory: pipeLazy({ maxMicroTaskTicks: 1 }, createVirtualTimeScheduler),
}), toArray()), expectToThrow))), describe("catchError", test("source completes successfully", pipeLazy(pipe(1, fromValue(fromArrayT$4)), catchError(_ => fromValue(fromArrayT$4)(2)), toRunnable(), toArray(), expectArrayEquals([1]))), test("source throws, error caught and ignored", () => {
    const error = newInstance(Error);
    pipe(1, fromValue(fromArrayT$4), concatWith(concatT, pipe(error, returns, throws({ ...fromArrayT$4, ...mapT }))), catchError(ignore), toRunnable(), toArray(), expectArrayEquals([1]));
}), test("source throws, continues with second observable", () => {
    const error = newInstance(Error);
    pipe(1, fromValue(fromArrayT$4), concatWith(concatT, pipe(error, returns, throws({ ...fromArrayT$4, ...mapT }))), catchError(_ => fromValue(fromArrayT$4)(2)), toRunnable(), toArray(), expectArrayEquals([1, 2]));
}), test("source throws, catch throws", () => {
    const error = newInstance(Error);
    expectToThrow(() => pipe(1, fromValue(fromArrayT$4), concatWith(concatT, pipe(error, returns, throws({ ...fromArrayT$4, ...mapT }))), catchError(_ => {
        throw error;
    }), toRunnable(), toArray()));
})), test("combineLatest", pipeLazy(generate$3(incrementBy(2), returns(1), { delay: 2 }), takeFirst$2({ count: 3 }), zipWith(combineLatestT, pipe(generate$3(incrementBy(2), returns(0), { delay: 3 }), takeFirst$2({ count: 2 }))), toRunnable(), toArray(), expectArrayEquals([
    [3, 2],
    [5, 2],
    [5, 4],
    [7, 4],
    ...
    pipe(sub2, dispose());
    pipe(subject, getObserverCount, expectEquals(0));
})), test("exhaustMap", pipeLazy([fromArray$3()([1, 2, 3]), fromArray$3()([4, 5, 6]), fromArray$3()([7, 8, 9])], fromArray$3(), concatMap({ ...exhaustT, ...mapT$1 }, (x) => x), toRunnable(), toArray(), expectArrayEquals([1, 2, 3]))), describe("fromPromise", testAsync("when the promise resolves", async () => {
    ...
    scheduler.dispose();
})), test("genMap", pipeLazy(undefined, fromValue(fromArrayT$4), genMap({ ...concatAllT, ...fromIteratorT, ...mapT$1 }, function* (_) {
    yield 1;
    yield 2;
    yield 3;
}), toRunnable(), toArray(), expectArrayEquals([1, 2, 3]))), describe("merge", test("two arrays", pipeLazy(merge(pipe([0, 2, 3, 5, 6], fromArray$3({ delay: 1 })), pipe([1, 4, 7], fromArray$3({ delay: 2 }))), toRunnable(), toArray(), expectArrayEquals([0, 1, 2, 3, 4, 5, 6, 7]))), test("when one source throws", pipeLazy(pipeLazy([1, 4, 7], fromArray$3({ delay: 2 }), concatWith(mergeT, throws({ ...fromArrayT$4, ...mapT }, { delay: 5 })(raise)), toRunnable(), last()), expectToThrow))), describe("mergeMap", test("when a mapped observable throws", pipeLazy(pipeLazy([
    fromArray$3({ delay: 1 })([1, 2, 3]),
    throws({ ...fromArrayT$4, ...mapT }, { delay: 2 })(raise),
], fromArray$3(), concatMap({ ...mergeAllT, ...mapT$1 }, identity), toRunnable(), last()), expectToThrow)), test("when the map function throws", pipeLazy(pipeLazy([1, 2, 3, 4], fromArray$3(), concatMap({ ...mergeAllT, ...mapT$1 }, (x) => {
    if (x > 2) {
        raise();
    }
    return fromValue(fromArrayT$4)(x);
}), toRunnable(), last()), expectToThrow))), test("never", pipeLazy(never(), toRunnable(), last(), expectNone)), test("observable", () => {
    const fromValueWithDelay = (delay, value) => fromValue(fromArrayT$4, { delay })(value);
    const emptyDelayed = empty(fromArrayT$4, { delay: 100 });
    ...
    const createOneTwoThree = (x) => isSome(x) ? fromArray$3()([1, 2, 3]) : empty(fromArrayT$4);
    pipe(observable(() => {
    ...
    const disp = mockFn();
    const f = mockFn(disp);
    pipe(1, fromValue(fromArrayT$4), onSubscribe(f), subscribe(scheduler));
    ...
    const scheduler = createVirtualTimeScheduler();
    const subscription = pipe(1, fromValue(fromArrayT$4), onSubscribe(raise), subscribe(scheduler));
...
})), describe("scanAsync", test("fast lib, slow acc", pipeLazy([1, 2, 3], fromArray$3(), scanAsync$1((acc, x) => fromValue(fromArrayT$4, { delay: 4 })(x + acc), returns(0)), toRunnable(), toArray(), expectArrayEquals([1, 3, 6]))), test("slow lib, fast acc", pipeLazy([1, 2, 3], fromArray$3({ delay: 4 }), scanAsync$1((acc, x) => fromValue(fromArrayT$4)(x + acc), returns(0)), toRunnable(), toArray(), expectArrayEquals([1, 3, 6]))), test("slow lib, slow acc", pipeLazy([1, 2, 3], fromArray$3({ delay: 4 }), scanAsync$1((acc, x) => fromValue(fromArrayT$4, { delay: 4 })(x + acc), returns(0)), toRunnable(), toArray(), expectArrayEquals([1, 3, 6]))), test("fast lib, fast acc", pipeLazy([1, 2, 3], fromArray$3(), scanAsync$1((acc, x) => fromValue(fromArrayT$4)(x + acc), returns(0)), toRunnable(), toArray(), expectArrayEquals([1, 3, 6])))), test("share", () => {
    const scheduler = createVirtualTimeScheduler();
...
}), describe("switchAll", test("with empty source", pipeLazy(empty(fromArrayT$4), switchAll(), toRunnable(), toArray(), expectArrayEquals([]))), test("when source throw", pipeLazy(pipeLazy(raise, throws({ ...fromArrayT$4, ...mapT }), switchAll(), toRunnable(), toArray(), expectArrayEquals([])), expectToThrow))), test("switchMap", pipeLazy([1, 2, 3], fromArray$3({ delay: 1 }), concatMap({ ...switchAllT, ...mapT$1 }, _ => pipe([1, 2, 3], fromArray$3())), toRunnable(), toArray(), expectArrayEquals([1, 2, 3, 1, 2, 3, 1, 2, 3]))), describe("takeLast", test("when pipeline throws", pipeLazy(pipeLazy(raise, throws({ ...fromArrayT$4, ...mapT }), takeLast$2(), toRunnable(), last()), expectToThrow))), describe("throttle", test("first", pipeLazy(generate$3(increment, returns(-1), { delay: 1 }), takeFirst$2({ count: 100 }), throttle(50, { mode: "first" }), toRunnable(), toArray(), expectArrayEquals([0, 49]))), test("last", pipeLazy(generate$3(increment, returns(-1), { delay: 1 }), takeFirst$2({ count: 200 }), throttle(50, { mode: "last" }), toRunnable(), toArray(), expectArrayEquals([49, 99, 149, 199]))), test("interval", pipeLazy(generate$3(increment, returns(-1), { delay: 1 }), takeFirst$2({ count: 200 }), throttle(75, { mode: "interval" }), toRunnable(), toArray(), expectArrayEquals([0, 74, 149, 199]))), test("when duration observable throws", pipeLazy(pipeLazy([1, 2, 3, 4, 5], fromArray$3({ delay: 1 }), throttle(_ => throws({ ...fromArrayT$4, ...mapT })(raise)), toRunnable(), last()), expectToThrow))), describe("throwIfEmpty", test("when source is empty", pipeLazy(pipeLazy(empty(fromArrayT$4), throwIfEmpty(() => undefined), toRunnable(), last()), expectToThrow)), test("when source is not empty", pipeLazy(1, returns, compute({ ...fromArrayT$4, ...mapT }), throwIfEmpty(() => undefined), toRunnable(), last(), expectEquals(1)))), describe("timeout", test("throws when a timeout occurs", pipeLazy(pipeLazy(1, fromValue(fromArrayT$4, { delay: 2 }), timeout(1), toArray()), expectToThrow)), test("when timeout is greater than observed time", pipeLazy(1, fromValue(fromArrayT$4, { delay: 2 }), timeout(3), toRunnable(), last(), expectEquals(1)))), describe("toPromise", testAsync("when observable completes without producing a value", async () => {
    const scheduler = createHostScheduler();
    await pipe(pipe(empty(fromArrayT$4), toPromise(scheduler)), expectPromiseToThrow);
    scheduler.dispose();
    ...
    [2, 0],
    [3, 1],
], arrayEquality()))), test("when latest produces no values", pipeLazy([0], fromArray$3({ delay: 1 }), withLatestFrom(empty(fromArrayT$4), sum), toRunnable(), toArray(), expectArrayEquals([]))), test("when latest throws", () => {
    const error = newInstance(Error);
    pipe(pipeLazy([0], fromArray$3({ delay: 1 }), withLatestFrom(throws({ ...fromArrayT$4, ...mapT })(returns(error)), sum), toRunnable(), toArray(), expectArrayEquals([])), expectToThrowError(error));
    ...
    [2, 2],
    [3, 3],
], arrayEquality()))), test("when source throws", pipeLazy(pipeLazy(raise, throws({ ...fromArrayT$4, ...mapT }), zipWith(zipT, fromArray$3()([1, 2, 3])), map$3(([, b]) => b), toRunnable(), toArray()), expectToThrow))), test("zipLatestWith", pipeLazy([1, 2, 3, 4, 5, 6, 7, 8], fromArray$3({ delay: 1 }), zipWith(zipLatestT, pipe([1, 2, 3, 4], fromArray$3({ delay: 2 }))), map$3(([a, b]) => a + b), toRunnable(), toArray(), expectArrayEquals([2, 5, 8, 11]))), describe("zipWithLatestFrom", test("when source throws", pipeLazy(pipeLazy(throws({ ...fromArrayT$4, ...mapT })(raise), zipWithLatestFrom(fromValue(fromArrayT$4)(1), (_, b) => b), toRunnable(), last()), expectToThrow)), test("when other throws", pipeLazy(pipeLazy([1, 2, 3], fromArray$3({ delay: 1 }), zipWithLatestFrom(throws({ ...fromArrayT$4, ...mapT })(raise), (_, b) => b), toRunnable(), last()), expectToThrow)), test("when other completes first", pipeLazy([1], fromArray$3({ delay: 1 }), zipWithLatestFrom(fromArray$3()([2]), (_, b) => b), toRunnable(), last(), expectEquals(2)))), test("fromArray with no start delay", () => {
    const scheduler = createVirtualTimeScheduler();
    const publishTimes = [];
...
}), createZippableTests({ ...fromArrayT$5, generate: generate$4, map: map$4, toRunnable: toRunnable$3, ...zipT$2 }));

const tests$1 = describe("streamable", test("__stream", () => {
    const streamable = createStateStore(() => 0);
    const createLooper = (stream) => pipe([1, 2, 3], fromArray$3({ delay: 10 }), onNotify(x => pipe(returns(x), dispatchTo(stream))), ignoreElements(keepT$3));
    ...
    const scheduler = createVirtualTimeScheduler();
    let result = [];
    const scanStream = pipe(createLiftedStreamable(scan$4(sum, returns(0)), onNotify(x => {
        result.push(x);
    })), stream(scheduler));
    ...
    const scheduler = createVirtualTimeScheduler();
    const src = createLiftedStreamable(scan$4((acc, _) => acc + 1, returns(0)), takeFirst$2({ count: 3 }));
    let result = 0;
    const dest = pipe(createLiftedStreamable(scan$4((acc, _) => acc + 1, returns(0)), onNotify(v => {
        result = v;
    }), mapTo(mapT$1, none), startWith({ ...concatT$1, ...fromArrayT }, none)), stream(scheduler), sourceFrom(src));
    ...
    const scheduler = createVirtualTimeScheduler();
    const f = mockFn();
    const subscription = pipe(str, fromValue(fromArrayT), encodeUtf8({ ...mapT$1, ...deferT }), decodeWithCharset(), fromObservable(), toObservable$1(), reduce((acc, next) => acc + next, returns("")), onNotify(f), subscribe(scheduler));
    pipe(scheduler, forEach(ignore));
    pipe(f, expectToHaveBeenCalledTimes(1));
