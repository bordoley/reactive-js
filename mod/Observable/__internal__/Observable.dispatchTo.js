...
const Observable_dispatchTo = ((dispatcher) => {
    ...
    return (observable) => Observable_isEnumerable(observable)
        ? Enumerable_create(pipeLazy(observable, Observable_enqueue(dispatcher), invoke(EnumerableLike_enumerate), Disposable_onComplete(bindMethod(dispatcher, DispatcherLike_complete))), false)
        : lift(observable);
});
