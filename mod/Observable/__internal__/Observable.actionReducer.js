...
const Observable_actionReducer = (reducer, initialState, options) => (obs => Observable_createWithConfig((observer) => {
    ...
    return pipe(obs, Observable_scan(reducer, returns(acc)), Observable_mergeWith(pipe(acc, Optional_toRunnable())), Observable_distinctUntilChanged(options), invoke(ObservableLike_observe, observer));
}, obs));
