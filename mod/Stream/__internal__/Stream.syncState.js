...
const Stream_syncState = (onInit, onChange, options) => {
    ...
    return (stateStore) => {
        ...
        return pipe(stateStore, Observable_forkMerge(compose(Observable_takeFirst(), MulticastObservable_concatMap(onInit)), compose(throttleDuration > 0
            ? Observable_throttle(throttleDuration)
            : identity, Observable_pairwise(), MulticastObservable_concatMap(([oldValue, newValue]) => onChange(oldValue, newValue)))), Observable_dispatchTo(stateStore), Observable_subscribe(scheduler, {
